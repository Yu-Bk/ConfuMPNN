...
| **第十八轮 分层数据+逆加权（通用模型）** | **1BC8 全 6 臂命中**（极端正电+17 dev 0.37）、2LZM 目标1 修复（t1_cond 0.35）、新正电验证 1b24A01 泛化 4/5、H3 全过；**但逆加权 cap=5 过头 → 1UBQ 退化（0/5，dev 2.5-3.2）**——需降 cap 平衡再训 v5 | `待提交` |
