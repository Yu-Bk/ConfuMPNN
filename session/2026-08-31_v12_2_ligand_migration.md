...
- **近配体距离定义**（`--contact-cutoff` 默认 4.5Å）——**工具定位=残基-配体距离计算工具**：残基**任一骨架原子（N/Cα/C/O）到配体任一原子最近距离 <4.5Å** → 标"近配体"（level=人工fix(近配体)，建议人工决定）。4.5Å=重原子非键接触上界（氢键 2.7-3.3 / 盐桥 3-4 / 范德华 3.5-4.5）。近配体 ⊆ 口袋（Cα 是骨架原子之一）。⚠️ 边界：① 只含骨架原子**不含侧链**（parse_PDB 无侧链）→ 侧链介导的相互作用漏检；② **不预测相互作用种类**（氢键/盐桥等）→ "近配体"≠"有相互作用"，真实功能接触需 PLIP/人工确认
...
3. **⛔ 更正：非"删减转移"，是"删减全局性病态，fix 只堵一个出口"**（分区域 `analyze_deletion_transfer.py`，commit 待）：
   | 2FEO 区域 | native带 | 无fix | 倍率 | 有fix | 倍率 |
   |------|:---:|:---:|:---:|:---:|:---:|
   | 深部fix | 10 | 1.4 | 0.14 | 10.0 | **1.00** ✅ |
   | 口袋表面带 | 2 | 0.4 | 0.20 | 0.4 | 0.18 |
   | 口袋外表面 | 39 | 27.2 | 0.70 | 26.6 | 0.68 |
   | 口袋外深部 | 5 | 0.5 | 0.10 | 0.6 | 0.13 |
   **fix 前后其余区域倍率几乎不变** → 删减本来就全局分布（深部 0.1-0.2× 最狠、表面 0.65-0.74×），fix 只保住深部出口（全序列增加全部来自此 +8.6）；**表面删减是 v12 配体模式固有，fix 前后不变**（非"失去控制"）→ 根治全局删减需训练侧组成监督
