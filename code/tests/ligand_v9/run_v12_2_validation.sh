#!/usr/bin/env bash
# v12.2 校准版泛化验证：采样 → ESMFold 回折 → TM-score → 统计 → PROPKA(H4)。
#
# 背景：v12.2（v12.1 + λ_target 0.2）闭环诊断达标——未校准 valid 区内 1.61、
#   校准表 charge_calibration_v12_2.json（global 1.579/−3.139）修到 1.00。
#   本脚本在泛化 10 蛋白上验证 v12.2+校准的完整有效性：
...
#
# 用法（后台）：nohup bash code/tests/ligand_v9/run_v12_2_validation.sh > log/v12_2_validation.stdout 2>&1 &
...
CAL="output/charge_calibration_v12_2.json"
OUT="output/generalization_v12_2_calib"
TARGET=50   # 10 蛋白 × 5 臂
...
PYTHONPATH=code "$PY" code/tests/ligand_v9/generalization_stats.py \
  --root "$OUT" --manifest "$MANIFEST" --out output/generalization_v12_2_calib_stats.json \
  > log/v12_2_val_stats.log 2>&1
log "V4 统计完成：output/generalization_v12_2_calib_stats.json"

# ================= V5: PROPKA 物理复核（H4）=================
log "== V5 PROPKA 复核（4 蛋白 × native/n8）=="
PYTHONPATH=code "$PY" - <<'PYEOF'
import json, subprocess, os
ROOT = 'output/generalization_v12_2_calib'
PY = '/home/baokun_yu/miniconda3/envs/confumpnn/bin/python'
PROTS = ['1BJ4', '1A65', '1AG0', '1C6O']   # 覆盖最坏响应/长蛋白/中等
outdir = 'output/propka_v12_2'
...
PYEOF
log "V5 PROPKA 完成"
touch log/v12_2_validation.DONE
