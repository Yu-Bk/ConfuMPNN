"""v12.2 配体模式 Tm/Sol 无条件基线采样（LigandMPNN 专用）。

与 `sample_unconditioned.py`（MoMPNN/蛋白模式）**分开保存**，互不干扰。
作用：对泛化 10 蛋白（含配体上下文）不加电荷条件生成 n 条序列，
分离"电荷条件化的物理性质代价" vs "逆折叠本身的固有代价"。
...
用法（项目根）：
  PYTHONPATH=code python code/tests/ligand_v9/sample_unconditioned_ligand.py \
      --manifest data/validation_pdbs/validation_manifest.json --out_dir output/tm_sol_ligand_v12_2/uncond \
...
def main():
    ...
    for it in man["items"]:
        ...
        cond_vec = make_condition_vector(args.pH, net_charge=CHARGE_MEAN)
        with open(fa, "w") as f:
