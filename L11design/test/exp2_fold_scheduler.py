#!/usr/bin/env python
"""L11 Exp2：ESMFold 回折调度器（confumpnn-esmfold 环境 + GPU，可 resume）。

扫描 data/exp2/*/seqs.fa（含 _native），对每个目录：
    已存在 plddt.csv 且行数==fasta 条数 → 跳过（resume）。
    否则回折全部序列 → dir/folds/*.pdb + dir/plddt.csv。

调度纪律：只使用 GPU util<12% 且 free mem >= --min_free_mem 的卡；单卡单任务。
用法（confumpnn-esmfold 环境）：
    python L11design/test/exp2_fold_scheduler.py [--min_free_mem 20000] [--poll 60]
"""
import argparse
import glob
import os
...
def main():
    ...
    while pending:
        ...
        while g is None:
            for idx, (util, free) in nvidia().items():
                if util < 12 and free >= args.min_free_mem:
                    ...
                    break
            if g is None:
                print(f"[{time.strftime('%H:%M:%S')}] 暂无空闲卡（需 util<12% & free>={args.min_free_mem}MiB），60s 后再试", flush=True)
        ...
        print(f"[{time.strftime('%H:%M:%S')}] 回折 {d.name} on GPU{g} (n={fasta_count(fa)})", flush=True)
