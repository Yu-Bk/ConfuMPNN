#!/bin/bash
# L11 Exp3/Exp4 CPU 打分（串行单进程版，避免多 Tm 进程争抢线程）。
#   Tm: 先 exp3 全部目录，再 exp4 全部目录（TemBERTure，一次一个进程）
#   Sol: Protein-Sol %sol（串行）→ data/exp3|exp4/*/seqs.fa-protein_sol.csv
# 用法：bash L11design/test/exp34_cpu_scores_serial.sh
set -u
