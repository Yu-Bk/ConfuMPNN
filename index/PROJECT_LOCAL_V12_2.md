...
- 判据：组成倍率 ∈ [0.7, 1.3]、slope ∈ [0.9, 1.15]、H2 单臂 dev≤2.0（DESIGN_CRITERIA 科学判据）
- **🚨 2026-09-03 废除 "H2 ≥ 当前 72%" 自我参照**：72% 原为 v12.2 per-protein（表内=已现场标定）自测成绩被误用作后续版及格线，属循环参照。达标判据只保留单臂 dev≤2.0；整体命中率按口径（global 未标定 / 小样本现场标定）分列报告，不做自我参照二分。详见 `session/2026-09-03_validation_refactor.md`
