...
| **v10 修复包（v11 路线）** | `index/v10_repair/` | v10 退化修复三步走：诊断脚本（`v10_diag_response_curve.py`）+ 分段分析（`analyze_diag.py`）+ train_finetune v11 三处补丁（`train_finetune_v11_patch.md`）｜**2026-08-28 已执行诊断**：结论修正为"训练覆盖区内响应增益失控（负向超线性），非纯外推"，需 B 降权+消融 | 2026-08-28 |
| **v10 响应曲线诊断报告** | `analysis/report/2026-08-28_v10_diag_response.md` | 17 蛋白（训练域 7+验证 10）× 11 targets × n20 响应曲线：全区/区内/区外 slope；外推假说未坐实（区内 1.48/1.67>1）；正负不对称（负向超线性过冲）；归因 B/C 叠加+decouple 弱锚；修正 v10 验证报告 §5"只改 decouple"错误（实测三组件全开）| 2026-08-28 |
