...
> **状态**：实测完成。**fix 100% 保住深部带电残基，口袋删减大幅缓解（2FEO -77%→-12%、1AXW -59%→-26%），但删减转移**（全序列仍 0.66-0.68×，模型改删别处）；2FEO 电荷 dev 恶化（1.4→4.0，校准失配）。结论：**fix=保深部功能残基的有效手段，非根治全局删减的手段**。
...
3. **但删减转移**：全序列组成仍 0.66-0.68×（无fix 0.54-0.59）——**fix 深部后模型把删减转移到未覆盖区域**（表面带电 + 口袋外深部）→ fix 保住了功能关键残基，但**全局删减捷径本身未根治**（需训练侧组成监督）
...
2. **新增认识（实测暴露）**：
   - fix 后删减会**转移**到未覆盖区域 → 工具提示可加"只 fix 深部带电时删减转移风险"
