...
> **定位**：配体模式中"**长 × 大电荷变化（深负为主）可设计性有限**"档的补充证据（与蛋白模式 1A65 同档）。
> 以 2E9R_X（FMDV RdRp, L476, native −10.1，长负 RNA 聚合酶）为配体模式代表：核糖体 RNA 结合蛋白
> 天然正电，而 2E9R_X 为长负聚合酶，负向工程化时系统性过冲 → 属模型生成侧能力边界，非 RNA 结合能力问题。
> 本报告归档该档证据与数据路径，供论文"能力边界"章节引用；标准测试集（in-10）不含该档蛋白。
...
### 3.1 为什么单列（能力边界档，非 RNA 结合问题）
- 核糖体 RNA 结合蛋白（如 5O60_E/21KL_A）**天然正电**；2E9R_X 是**长负 RNA 聚合酶**，
